...
export default class PersonTablePage extends PageWithTable {
  ...
  _init(model) {
    let m = $.extend({}, this._jsonModel(), model);
    super._init(m);
    this._initListeners();
  }
  ...
  _loadTableData(searchFilter) {
    let restriction = scout.create('helloscout.PersonRestriction', searchFilter, {
  ...
  _createPersonForm() {
    ...
    let personForm = scout.create('helloscout.PersonForm', {
      parent: outline
    });
    return personForm;
  }
  ...
  _onCreatePersonMenuAction(event) {
    let personForm = this._createPersonForm();
    let emptyPerson = scout.create('helloscout.Person', {}, {
      ensureUniqueId: false
    });
